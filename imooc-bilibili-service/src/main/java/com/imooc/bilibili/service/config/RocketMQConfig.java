    ...
    public DefaultMQPushConsumer momentsConsumer() throws Exception{
        ...
        consumer.registerMessageListener(new MessageListenerConcurrently() {
            ...
            public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context){
                ...
                //msg的body转换成obj类，再进一步转换成usermoments
                ...
                for(UserFollowing fan : fanList){
                    String key = "subscribed-" + fan.getUserid();//获取粉丝的userid
                    String subscribedListStr = redisTemplate.opsForValue().get(key);//从redis中检索出来的数据
                    List<UserMoment> subscribedList;
                    if(StringUtil.isNullOrEmpty(subscribedListStr)){
                        subscribedList = new ArrayList<>();
                    }else{
                        subscribedList = JSONArray.parseArray(subscribedListStr, UserMoment.class);//json解析
                    }
    ...
    public DefaultMQPushConsumer danmusConsumer() throws  Exception{
        ...
        consumer.registerMessageListener(new MessageListenerConcurrently() {
            ...
            public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
                ...
                WebSocektService webSocektService = WebSocektService.WEBSOCKET_MAP.get(sessionId);
                if(webSocektService.getSession().isOpen()){
                    try {
                        webSocektService.sendMessage(message);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
                }
